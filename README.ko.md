...
Discord mention → MeetingRun 생성
  ...
  → opencode-go Worker 실행 (실제 작업 수행)
...
| `aicompanyceo` | `대표` | `#전략-회의실` | `1505600167221526621` | CEO/코디네이터: 회사 기본 진입점, 요청 라우팅, 최종 종합 보고서, 회의 개회/폐회 |
...
| `#전략-회의실` | 멀티봇 회의 조율, 최종 의사결정 보고서, 경영진 라우팅 |
...
src/runtime_architecture_v2/   ← 핵심 런타임 모듈
  ...
  workers.py                   Worker 실행 계층 (fake + opencode-go)
...
A: opencode-go는 별도 CLI 도구다. `which opencode`로 확인 가능하며, WorkerRunner가 subprocess로 호출한다.
