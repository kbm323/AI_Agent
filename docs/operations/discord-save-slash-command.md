...
유지하며 payload에 immutable cutoff를 기록한다. 더 늦은 `/save`는 같은 source/boundary의
...
# Stop every gateway that loaded the candidate before restoring disk state.
tmux kill-session -t hermes-aicompanyassistant 2>/dev/null || true
for profile in "${profiles[@]}"; do
  state_root="$ROLLBACK_STATE_DIR/$profile"
  session="hermes-${profile}"
  if [ -f "$state_root/loaded-by-deployment" ]; then
    tmux kill-session -t "$session" 2>/dev/null || true
  fi
done
...
# Start every restored profile for tool and native-picker absence checks.
tmux new-session -d -s hermes-aicompanyassistant -x 120 -y 40 \
  "HERMES_ACCEPT_HOOKS=1 hermes --profile aicompanyassistant gateway run"
for profile in "${profiles[@]}"; do
  test "$profile" = aicompanyassistant && continue
...
`/tools`에 `save_discord_thread_to_obsidian`가 없고 native picker에 `/save`가 없다는
...
for profile in "${profiles[@]}"; do
  state_root="$ROLLBACK_STATE_DIR/$profile"
  session="hermes-${profile}"
  test -s "$DEPLOY_RECORD_DIR/$profile.rollback-absence.txt"
  grep -F "tool absent: save_discord_thread_to_obsidian" \
    "$DEPLOY_RECORD_DIR/$profile.rollback-absence.txt"
  grep -F "picker absent: /save" \
    "$DEPLOY_RECORD_DIR/$profile.rollback-absence.txt"
