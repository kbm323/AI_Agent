...
**Architecture:** Keep Discord registration in the existing `ai-agent-commands` plugin and put all behavior in transport-neutral Runtime v2 modules. Store immutable source/note records and mutable canonical pages in the existing `raw/` and `wiki/` trees; execute QMD with argument arrays through an injectable adapter; persist a dirty marker and serialize index work across all seven gateway processes.

**Tech Stack:** Python 3.11, pytest/pytest-asyncio, Hermes Agent plugin API, standard-library HTTP/HTML parsing, `yt-dlp` CLI for YouTube metadata/transcripts, QMD CLI, Obsidian Markdown, existing Runtime v2 sanitization and locking patterns.
...
- Invoke QMD and `yt-dlp` with `shell=False`; never interpolate user text into a shell command.
...
- Missing models, QMD, `yt-dlp`, inaccessible pages, malformed output, and timeouts fail closed with sanitized Korean responses.
...
- Create `src/runtime_architecture_v2/llmwiki_sources.py`: single-URL parsing, URL normalization, SSRF-safe generic web retrieval, and YouTube retrieval through `yt-dlp`.
...
### Task 4: Safe Multi-Source URL Retrieval

**Files:**
- Create: `src/runtime_architecture_v2/llmwiki_sources.py`
- Create: `tests/test_runtime_architecture_v2_llmwiki_sources.py`

**Interfaces:**
- Consumes: `LlmWikiSource` from `llmwiki_models.py` in Task 3.
- Produces: `extract_single_url(text: str) -> str`, `normalize_source_url(url: str) -> str`, and `SourceRetriever.retrieve(url: str) -> LlmWikiSource`.
- Initial source classes: YouTube video metadata/transcripts through `yt-dlp`; generic public HTTP(S) pages, articles, and posts through the standard-library fetcher.

- [ ] **Step 1: Write failing classification and security tests**
...
def test_generic_retriever_rejects_private_targets_and_redirects():
    with pytest.raises(SourceError, match="unsafe_target"):
        SourceRetriever(fetcher=FakeFetcher()).retrieve("http://127.0.0.1/private")
...
Expected: import failure for `llmwiki_sources`.

- [ ] **Step 3: Implement bounded retrieval**

Allow only `http` and `https`. Resolve and validate every destination and redirect; reject loopback, private, link-local, multicast, reserved, and unspecified addresses. Validate the connected peer address again before reading the body to close DNS-rebinding gaps. Limit redirects to 5, body size to 10 MiB, and wall time to 30 seconds. Accept text, HTML, JSON, and subtitle content types. Strip scripts/styles and collapse whitespace with `html.parser`. For recognized YouTube video hosts, run `yt-dlp --dump-single-json --skip-download --write-auto-subs --write-subs --sub-langs ko,en --sub-format vtt` in a temporary directory through an injected runner, parse the generated VTT, and retain metadata/transcript text only. Public YouTube posts and other public posts use the generic page path. Links that require login, execute-only JavaScript, or return unsupported media fail with `unsupported_source`.

- [ ] **Step 4: Run security and retrieval tests**

Run: `.venv\Scripts\python.exe -m pytest tests/test_runtime_architecture_v2_llmwiki_sources.py -q`

Expected: all tests pass, including redirect revalidation, oversized bodies, malformed `yt-dlp` output, and timeout cases.
...
git commit -m "feat: add safe LLM Wiki source retrieval"
```
...
- [ ] **Step 3: Implement command services and structured summarization**
...
- [ ] **Step 4: Verify command services**
...
- [ ] **Step 5: Commit command services**
...
- [ ] **Step 1: Extend plugin tests before registration code**
...
- [ ] **Step 3: Register handlers without domain duplication**
...
- [ ] **Step 4: Verify plugin and archive regressions**
...
- [ ] **Step 1: Write failing reconciliation and runbook guard tests**
...
- [ ] **Step 3: Add reconciliation CLI and exact deployment procedure**

Document and later execute these gates in order: verify Node.js >=22; install official `@tobilu/qmd` package; verify QMD on Ubuntu ARM64; export the pinned Qwen3 model; add the `obsidian` collection once; run `qmd update` and `qmd embed -f`; verify one Korean JSON query; verify `yt-dlp`; install a systemd oneshot service and five-minute timer invoking the reconciliation CLI; deploy one reviewed plugin revision to Assistant; run all three commands and `/archive`; then deploy the same hash to the remaining six profiles sequentially. Rollback disables the timer, restores the prior plugin revision, and leaves the vault and QMD cache intact.

- [ ] **Step 4: Run full verification before live deployment**
...
- [ ] **Step 5: Commit operational support**
...
- [ ] URL retrieval covers accessible generic pages/articles/posts and YouTube metadata/transcripts without downloading media.
