...
from src.runtime_architecture_v2.pilot import Phase13PilotModeError  # noqa: E402


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Phase 14 multi-bot operational protocol pilot."
    )
    parser.add_argument(
        "--mode",
        ...
        default="dry-run",
        help=(
            "dry-run never calls opencode-go; live-worker permits up to "
            "2 live workers."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="1 or 2 for --mode live-worker; must be 0 in dry-run.",
    )
    parser.add_argument(
        "--live-discord",
        action="store_true",
        help=(
            "Actually publish bot messages through LiveDiscordProjectionSink; "
            "requires --mode live-worker."
        ),
    )
    parser.add_argument(
        "--target-channel-id",
        default="phase14-channel",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            result.to_cli_dict(), ensure_ascii=False, sort_keys=True, indent=2
        )
    )
