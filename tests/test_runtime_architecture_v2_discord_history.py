...
def test_failed_page_checkpoint_redacts_complete_yaml_credential_scalars(tmp_path):
    checkpoint_root = tmp_path / "collection"
    first_page = [_message(str(i)) for i in range(249, 149, -1)]
    first_page[0]["content"] = (
        "password: 'checkpoint''s secret'\n"
        "token: plain checkpoint secret\n"
        "credential: |-\n"
        " literal checkpoint secret\n"
        "auth: >+1\n"
        " folded checkpoint secret\n"
